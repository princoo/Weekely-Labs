...
export default function WatchListBadge() {
    const {favorites:watchlistCount} = useAppSelector((state) => state.watchlist)
  return (
            <Link
              to="watchlist"
              className="relative flex items-center space-x-2 text-gray-300 hover:text-secondary transition-colors group"
            >
